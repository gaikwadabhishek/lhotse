...
class AudioSamples(BatchIO):
    ...
    def __init__(
        ...
        if self._metrics_dir or collect_metrics:
            ...
            self._tar_io_object_latencies: Optional[List[float]] = []
            self._tar_io_batch_latencies: Optional[List[Tuple[float, int]]] = []
        else:
            self.collect_metrics = False
            self._object_latencies = None
            self._batch_latencies = None
            self._tar_io_object_latencies = None
            self._tar_io_batch_latencies = None
    ...
    def __call__(
        ...
        # Extract tar I/O latencies from cuts (set by LazySharIterator)
        # before they get modified by collate_audio's padding.
        if self.collect_metrics:
            tar_io_sum = 0.0
            tar_io_count = 0
            for cut in cuts:
                lat = getattr(cut, "_tar_io_latency", None)
                if lat is not None:
                    self._tar_io_object_latencies.append(lat)
                    tar_io_sum += lat
                    tar_io_count += 1
            if tar_io_count > 0:
                self._tar_io_batch_latencies.append((tar_io_sum, tar_io_count))
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        # Tar I/O latencies (only present when data comes from LazySharIterator)
        if self._tar_io_object_latencies:
            tar_obj = np.array(self._tar_io_object_latencies)
            result["tar_io_per_object"] = {
                "p50": float(np.median(tar_obj)),
                "p95": float(np.percentile(tar_obj, 95)),
                "p99": float(np.percentile(tar_obj, 99)),
                "avg": float(np.mean(tar_obj)),
            }
        else:
            result["tar_io_per_object"] = {
                "p50": 0.0,
        ...
        if self._tar_io_batch_latencies:
            tar_batch_sums = np.array([t for t, _ in self._tar_io_batch_latencies])
            result["tar_io_batch"] = {
                "p50": float(np.median(tar_batch_sums)),
                "p95": float(np.percentile(tar_batch_sums, 95)),
                "p99": float(np.percentile(tar_batch_sums, 99)),
                "avg": float(np.mean(tar_batch_sums)),
            }
        else:
            result["tar_io_batch"] = {
                "p50": 0.0,
    ...
    def reset_metrics(self) -> None:
        ...
        if self.collect_metrics:
            ...
            self._tar_io_object_latencies = []
            self._tar_io_batch_latencies = []
